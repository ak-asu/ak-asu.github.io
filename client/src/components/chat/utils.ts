...
export const getSimulatedResponse = (userMessage: string): string => {
  ...
  if (lowerCaseMessage.includes('hello') || lowerCaseMessage.includes('hi') || lowerCaseMessage.includes('hey')) {
  ...
  } else if (lowerCaseMessage.includes('skill') || lowerCaseMessage.includes('technology') || lowerCaseMessage.includes('tech stack')) {
    ...
    work.forEach(job => {
      ...
      const duration = job.startDate.split('-')[0] + '-' + endDate.split('-')[0];
    ...
    return response;
  } else if (lowerCaseMessage.includes('education') || lowerCaseMessage.includes('degree') || lowerCaseMessage.includes('university')) {
    ...
    const courses = education.flatMap(edu => edu.subjects.map(s => s.name)).slice(0, 3);
    response += `\nI've completed coursework in ${courses.join(', ')}, and more.`;
    return response;
  } else if (lowerCaseMessage.includes('achievement') || lowerCaseMessage.includes('award') || lowerCaseMessage.includes('recognition')) {
    let response = `Some of my notable achievements include:\n\n`;
    achievements.forEach(achievement => {
      response += `• **${achievement.title}** - ${achievement.description}\n\n`;
    });
    return response;
  } else if (lowerCaseMessage.includes('about') || lowerCaseMessage.includes('who') || lowerCaseMessage.includes('background')) {
