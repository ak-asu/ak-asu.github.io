...
export const Shuffle = () => {
  ...
  const [gameState, setGameState] = useState<
    "initial" | "memorize" | "flipping" | "shuffling" | "playing" | "won"
  >("initial");
  const [wins, setWins] = useState(0);
  const [attempts, setAttempts] = useState(0);
  const [announcement, setAnnouncement] = useState("");
  const shuffleTimersRef = useRef<NodeJS.Timeout[]>([]);
  const [swappingCards, setSwappingCards] = useState<[number, number] | null>(
    null,
  );
  ...
  const initializeGame = () => {
    ...
    setAnnouncement(
      `Game started. Find the card with letter ${randomAlphabet}.`,
    );
  ...
  const performShuffles = () => {
    ...
    shuffleTimersRef.current.push(
      setTimeout(
        ...
        numSwaps * shuffleSpeed + 500,
      ), // Additional time after last swap
    );
  };
  ...
  const handleCardClick = (card: CardType) => {
    ...
    if (card.isTarget) {
      ...
      setTimeout(() => {
        setCards((prev) =>
          prev.map((c) => ({ ...c, isFlipped: true, isRevealed: true })),
        );
      }, 500);
    } else {
  ...
  return (
    ...
    >
          ...
          >
            <motion.div
              className={`relative w-full h-full rounded-md overflow-hidden
                ${
                  swappingCards &&
                  (swappingCards[0] === card.id || swappingCards[1] === card.id)
      ...
      <div className="mb-4">
        <p>
          Wins: {wins} | Attempts: {attempts} | Difficulty:{" "}
          {getShuffleSpeedText()}
        </p>
      </div>

      <Button onClick={initializeGame} aria-label="Start new game">
        New Game
      </Button>
