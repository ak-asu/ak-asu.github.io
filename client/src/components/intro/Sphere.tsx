...
const CursorArrow = ({
  ...
  const points = [new THREE.Vector3(...from), to];

  // Create direction vector for arrow
  const dir = new THREE.Vector3()
    .subVectors(to, new THREE.Vector3(...from))
    .normalize();
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      meshRef.current.position.add(
        normalizedDirection.clone().multiplyScalar(speed),
      );
    }
  });
...
const SphereScene: React.FC<SphereSceneProps> = ({
  ...
  const handleMouseDown = useCallback(
    (e: any) => {
      if (e.button === 0 && isInside && !e.object.isBall) {
        // Left click and not on a ball
        ...
        for (let j = i + 1; j < updatedBalls.length; j++) {
          ...
          if (actualDistance < minDistance) {
            ...
            const impulse = collisionNormal
              .clone()
              .multiplyScalar(relativeVelocity.dot(collisionNormal));
        ...
        for (let i = 0; i < physicsBalls.length; i++) {
          ...
          if (distance < (physicsBalls[i]?.size || 0.5) + 0.1) {
            ...
            setPhysicsBalls((prevBalls) => {
              ...
              const impulse = bullet.direction
                .clone()
      ...
      <group ref={groupRef}>
        ...
        {physicsBalls.map((ball, index) => (
          <Ball
            ...
            speed={ball.speed}
            url={
              ball.logoType === "github"
                ? "https://github.com"
                : ball.logoType === "linkedin"
                  ? "https://linkedin.com"
                  : undefined
            }
          />
